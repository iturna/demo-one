...
contract InstaArb is Ownable {
  ...
  // }
    ...
    function swap(address router, address _tokenIn, address _tokenOut, uint256 _amount) external onlyOwner {
        _swap(router, _tokenIn, _tokenOut, _amount);
    }
  ...
  function dualDexTrade(address _router1, address _router2, address _token1, address _token2, uint256 _amount) external onlyOwner {
    ...
    _swap(_router2,_token2, _token1,tradeableAmount);
  ...
  */ 
  ...
  // function getTokenBalances(address _owner, address[] memory _tokenContracts) external view returns (uint256[] memory) {
  //     uint256[] memory balances = new uint256[](_tokenContracts.length);
      
  //     // Loop through each token contract and get the balance
  //     for (uint i = 0; i < _tokenContracts.length; i++) {
  //         balances[i] = IERC20(_tokenContracts[i]).balanceOf(_owner);
  //     }
